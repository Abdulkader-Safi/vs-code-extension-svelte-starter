...
<div class="max-w-2xl p-6">
  <header class="mb-6">
    ...
    <p class="text-sm text-vscode-description">
      Send a message from the webview to VS Code as an information notification.
    </p>
  </header>
  ...
  <section class="p-3 mt-8 text-sm rounded-sm bg-vscode-section-bg">
    ...
    <p class="text-vscode-description">
      The button posts a
      <code class="px-1 py-0.5 rounded font-mono text-xs bg-vscode-code-bg">showNotification</code>
      message to the extension host, which calls
      <code class="px-1 py-0.5 rounded font-mono text-xs bg-vscode-code-bg">vscode.window.showInformationMessage()</code>.
    </p>
  </section>
</div>
